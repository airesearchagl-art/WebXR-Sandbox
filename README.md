...
Quest Touch Plus Controllerで実機実測したbutton mappingに基づき、以下の操作を割り当てています（`pressed`の立ち上がりエッジで発火。押しっぽなしで連続実行はされません）。

| 操作 | Controller | button index | 実測ボタン名 |
| :--- | :--- | :--- | :--- |
| 次のシーン (`nextScene`) | Right | `#4` | Right A |
| 前のシーン (`prevScene`) | Left | `#4` | Left X |
| HUD表示ON/OFF (`toggleHud`) | Right | `#5` | Right B |
| Debug詳細表示ON/OFF (`toggleDebugDetail`) | Left | `#5` | Left Y |

- Trigger（`#0`）・Grip（`#1`）は現時点では未割り当てで、将来の拡張用に予約しています。
- Left Menu（`#12`）はDebug Panelの取得ログ（pressed/touched一覧）には表示されますが、主要操作には割り当てていません。
- ダミーシーンは3種類（Scene 1: 赤Cube / Scene 2: 青Cube / Scene 3: 緑Cube＋背景色変更）で、別々のTHREE.Sceneを持つのではなく、単一シーン内でCubeの色と背景色を切り替える方式です。
